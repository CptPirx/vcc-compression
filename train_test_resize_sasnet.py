# %%
SELECTED_GPUS = [3, 4, 5, 6]  # which GPUs to use
...
# File logger
fh = logging.FileHandler(f'resized_results/{str(datetime.now()).replace(":", "_")}.log')
...
class CrowdCountingDataset(torch.utils.data.Dataset):
    def __init__(self, split, model_input_size, resize_alg):
        self.split = split
    ...
    def __getitem__(self, index):
        ...
        if self.resize_alg == 'bilinear':
            resample = Image.BILINEAR
        elif self.resize_alg == 'bicubic':
            resample = Image.BICUBIC
        elif self.resize_alg == 'lanczos':
            resample = Image.LANCZOS
        else:
            resample = Image.NEAREST

        image = image.resize((self.model_input_size[1], self.model_input_size[0]), resample=resample)
...
def get_model_path(config):
    return 'resized_results/sasnet_%d_%d_%s.pth' % (config['input_size'][0], config['input_size'][1], config['version'])


def train(config):
    # create datasets and data loaders
    train_dataset = CrowdCountingDataset('train', config['input_size'], config['version'])
    ...
    val_dataset = CrowdCountingDataset('val', config['input_size'], config['version'])
    ...
    test_dataset = CrowdCountingDataset('test', config['input_size'], config['version'])
...
config = {
    ...
    'wd': 1e-4,
    'epochs': 20,
    # 'version': ['bilinear', 'bicubic', 'lanczos'],
    'version': ['nearest'],
}
...
for i, s in enumerate(scale):
    logger.info('_' * 20)
    logger.info(f'Run {i + 1}/{len(scale)}')
    logger.info(f'Scale: {s * 10}%')
    run_config = config.copy()
    run_config['input_size'] = tuple([int(x * (s / 10)) for x in config['input_size']])

    for alg in config['version']:
        logger.info('_' * 10)
        run_config['version'] = alg
        logger.info(f'Config: {run_config}')
        train(run_config)

#     # One time experiment to save image sizes
#     image = Image.open('../../data/shanghai_tech_cc/part_B_final/train_data/images/IMG_1.jpg').convert('RGB')
#     if i == 0:
#         resized_image = np.asarray(image)
#         print(resized_image.nbytes / 1000)
#
#     resized_image = image.resize((run_config['input_size'][1], run_config['input_size'][0]))
#     resized_image = np.asarray(resized_image)
#     compressed_image_size = resized_image.nbytes / 1000
#     image_sizes.append(float(compressed_image_size))
#
# with open('resized_results/image_sizes.yaml', 'w') as f:
